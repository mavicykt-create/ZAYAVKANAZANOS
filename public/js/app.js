...
function showMainMenu() { currentView = 'menu'; renderApp(); checkShiftStatus(); loadLastSyncDate(); loadPendingClaimsCount(); checkInstallPrompt(); }
...
function renderClaimDetailContent(claim) {
  ...
  if (claim.status === 'pending') {
  ...
  } else if (claim.status === 'in_progress' && claim.assigned_to === currentUser.id) {
    html += `<div class="claim-detail-card">
      ...
      <div style="margin-bottom: 12px;">
        <label style="font-size: 13px; color: var(--ios-gray); display: block; margin-bottom: 4px;">Доказательства (фото/видео)</label>
        <input type="file" id="claimEvidenceFile" accept="image/*,video/*" onchange="handleEvidenceFile(this)">
...
function renderAdmin() {
  ...
  const tabsHtml = `
    <div class="admin-tabs">
      ...
      ${isAdmin ? `<div class="admin-tab ${adminActiveTab === 'claims' ? 'active' : ''}" onclick="switchAdminTab('claims')">Претензии</div>` : ''}
    </div>
  `;
  ...
  if (adminActiveTab === 'overview') {
    ...
    `;
  } else if (adminActiveTab === 'claims') {
    contentHtml = `
      <div class="admin-card">
        <div class="admin-card-title">Создать претензию</div>
        <div style="margin-bottom: 16px;">
          ...
          <input type="text" id="claimOrderInfo" class="form-input" placeholder="Заказ / Сборка с экрана" style="margin-bottom: 8px;">
          ...
          <div style="margin-bottom: 12px;">
            <label style="font-size: 13px; color: var(--ios-gray); display: block; margin-bottom: 4px;">Вложение (изображение или документ)</label>
            <input type="file" id="claimAttachment" accept="image/*,.pdf,.doc,.docx" onchange="handleClaimAttachment(this)">
            <div id="claimAttachmentName" style="font-size: 12px; color: var(--ios-green); margin-top: 4px;"></div>
          </div>
          <button class="btn btn-primary" onclick="createClaim()" style="width: 100%;">➕ Создать претензию</button>
        </div>
      </div>
      <div class="admin-card">
        <div class="admin-card-title">Список претензий</div>
        <div id="adminClaimsList"><div class="loading">Загрузка...</div></div>
      </div>
    `;
  }
...
function switchAdminTab(tab) {
  ...
  if (tab === 'claims') loadAdminClaimsList();
}
...
// ===== ADMIN CLAIMS FUNCTIONS =====

let claimAttachmentData = null;
let claimAttachmentType = null;
let claimAttachmentName = null;

function handleClaimAttachment(input) {
  const file = input.files[0];
  if (!file) return;

  claimAttachmentName = file.name;
  claimAttachmentType = file.type;
  document.getElementById('claimAttachmentName').textContent = `Выбран: ${file.name}`;

  const reader = new FileReader();
  reader.onload = function(e) {
    claimAttachmentData = e.target.result;
  };
  reader.readAsDataURL(file);
}

async function createClaim() {
  const checkNumber = document.getElementById('claimCheckNumber')?.value?.trim() || '';
  const purchaseTime = document.getElementById('claimPurchaseTime')?.value?.trim() || '';
  const orderInfo = document.getElementById('claimOrderInfo')?.value?.trim() || '';
  const missingProducts = document.getElementById('claimMissingProducts')?.value?.trim() || '';
  const claimText = document.getElementById('claimText')?.value?.trim() || '';

  if (!claimText) {
    alert('Укажите суть претензии');
    return;
  }

  const payload = { checkNumber, purchaseTime, orderInfo, missingProducts, claimText };

  if (claimAttachmentData) {
    payload.attachmentData = {
      fileData: claimAttachmentData,
      fileType: claimAttachmentType,
      fileName: claimAttachmentName
    };
  }
  ...
  if (result.success) {
    ...
    // Clear form
    document.getElementById('claimCheckNumber').value = '';
    document.getElementById('claimPurchaseTime').value = '';
    document.getElementById('claimOrderInfo').value = '';
    document.getElementById('claimMissingProducts').value = '';
    document.getElementById('claimText').value = '';
    document.getElementById('claimAttachment').value = '';
    document.getElementById('claimAttachmentName').textContent = '';
    claimAttachmentData = null;
    claimAttachmentType = null;
    claimAttachmentName = null;
    loadAdminClaimsList();
...
async function loadAdminClaimsList() {
  const container = document.getElementById('adminClaimsList');
  if (!container) return;

  const data = await apiGet('/api/claims/list');
  if (!data || data.length === 0) {
    container.innerHTML = '<div class="empty-state">Нет претензий</div>';
    return;
  }
  ...
  container.innerHTML = data.map(c => {
    ...
    return `<div class="claim-item">
      <div class="claim-header">
        <span class="claim-status ${statusClass}">${statusText}</span>
        <span class="claim-date">#${c.id} | ${new Date(c.created_at).toLocaleString('ru-RU')}</span>
      </div>
      <div class="claim-text">${c.claim_text}</div>
      ${c.assigned_to_name ? `<div class="claim-assignee">👤 ${c.assigned_to_name}</div>` : ''}
      ${c.check_number ? `<div class="claim-meta">Чек: ${c.check_number}</div>` : ''}
      <button class="btn btn-danger" onclick="deleteClaim(${c.id})" style="margin-top: 8px; width: 100%;">🗑️ Удалить</button>
    </div>`;
  }).join('');
}
...
async function deleteClaim(claimId) {
  ...
  const result = await apiPost(`/api/claims/${claimId}/delete`, {});
  if (result.success) {
    loadAdminClaimsList();
    loadPendingClaimsCount();
  } else {
