...
// Carry Products - показываем товары с общими количествами
async function showCarryProducts(categoryId, categoryName) {
  ...
  // Загружаем товары, мои заявки и статистику категории
  const [products, myRequests, categoryStats] = await Promise.all([
...
async function completeOrder() {
  ...
  // Помечаем все собранные товары
  const items = await apiGet('/api/carry/assembly-all');
  const productIds = Array.from(collectedIds);
  
  for (const productId of productIds) {
    await apiPost('/api/carry/toggle-collected', { productId, collected: true });
  }
  
  alert('Сборка завершена! Галочки сохранены для всех пользователей.'); 
