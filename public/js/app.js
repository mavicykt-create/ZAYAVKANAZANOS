...
function renderMainMenu() {
  ...
  return `<div class="main-layout">
    ...
    <div style="padding: 8px 16px; background: var(--ios-card); text-align: center;">
      <span id="lastSyncDate" style="font-size: 12px; color: var(--ios-gray);">ZAN 1.2 — Умный склад</span>
    </div>
    ...
    <div class="menu-grid">
      ...
      <div class="menu-item" onclick="showChat()" style="position: relative;">
        <div class="menu-icon">💬</div>
        <span class="menu-badge" id="chatBadge" style="display: none;"></span>
    ...
    <div id="todayTasks" style="padding: 8px 16px;"></div>
...
function showMainMenu() { 
  currentView = 'menu'; renderApp(); 
  checkShiftStatus(); 
  loadLastSyncDate(); 
  loadPendingClaimsCount(); 
  loadCalendarPendingDates(); 
  loadUnreadChatCount();
  loadTodayTodos();
  checkInstallPrompt(); 
}
...
async function loadUnreadChatCount() {
  const data = await apiGet('/api/profile/chat/unread-count');
  if (data) {
    unreadChatCount = data.count;
    updateChatBadge();
  }
}
...
function updateChatBadge() {
  ...
  if (chatBadge) {
    chatBadge.style.display = unreadChatCount > 0 ? 'flex' : 'none';
    chatBadge.textContent = unreadChatCount > 99 ? '99+' : unreadChatCount;
  }
}

async function loadTodayTodos() {
  const items = await apiGet('/api/calendar/todos-today-active');
  todayTodos = items || [];
  renderTodayTodos();
}
...
function renderTodayTodos() {
  ...
  container.innerHTML = `<div style="padding: 0 16px 8px; font-size: 13px; color: var(--ios-gray); font-weight: 600;">📋 Задачи на сегодня (${todayTodos.length})</div>` + 
    todayTodos.map(t => {
    let timerHtml = '';
    if (t.timer_deadline_at) {
      const deadline = new Date(t.timer_deadline_at);
      const diffMs = deadline - now;
      const diffHours = Math.floor(diffMs / (1000 * 60 * 60));
      const diffMinutes = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
      
      if (diffMs > 0) {
        const urgencyColor = diffHours < 2 ? 'var(--ios-red)' : diffHours < 8 ? 'var(--ios-orange)' : 'var(--ios-blue)';
        timerHtml = `<div style="font-size: 12px; color: ${urgencyColor}; font-weight: 600;">⏱️ ${diffHours}ч ${diffMinutes}м</div>`;
      } else {
        timerHtml = `<div style="font-size: 12px; color: var(--ios-red); font-weight: 600;">⏱️ Истёк!</div>`;
      }
    }
    
    return `<div class="today-task-item" onclick="toggleTodoAndRefresh(${t.id})">
      <div class="today-task-checkbox"></div>
      <div class="today-task-content">
        <div style="font-weight: 500; font-size: 14px;">${t.title}</div>
        ${t.description ? `<div style="font-size: 12px; color: var(--ios-gray);">${t.description}</div>` : ''}
        ${timerHtml}
      </div>
    </div>`;
  }).join('');
...
async function toggleTodoAndRefresh(todoId) {
  const result = await apiPost(`/api/calendar/todos/${todoId}/toggle`, {});
  if (result.error) {
    alert(result.error);
  }
  loadTodayTodos();
...
function renderAdmin() {
  ...
  return `<div class="main-layout">
    ...
    <div class="admin-nav">
      ...
      <div class="admin-tab ${adminActiveTab === 'push' ? 'active' : ''}" onclick="switchAdminTab('push')">Пуши</div>
      <div class="admin-tab ${adminActiveTab === 'scores' || adminActiveTab === 'score-weights' ? 'active' : ''}" onclick="switchAdminTab('scores')">🏆 Рейтинг</div>
...
async function loadAdminTab(tab) {
  ...
  switch (tab) {
    ...
    case 'holidays':
      ...
      break;
    case 'push':
      await loadAdminPush(content);
      break;
    case 'scores':
      await loadAdminScores(content);
      break;
    case 'score-weights':
      await loadAdminScoreWeights(content);
      break;
    case 'scale':
...
// ===== РЕЙТИНГ / БАЛЛЬНАЯ СИСТЕМА =====

async function loadAdminScores(content) {
  const today = new Date().toISOString().split('T')[0];
  const [report, weights] = await Promise.all([
    apiGet(`/api/scores/report?startDate=${today}&endDate=${today}`),
    apiGet('/api/scores/weights')
  ]);

  let html = `<div style="padding: 16px;">
    <div style="display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap;">
      <input type="date" id="scoreStart" class="form-input" style="width: auto;" value="${today}">
      <input type="date" id="scoreEnd" class="form-input" style="width: auto;" value="${today}">
      <button class="btn btn-primary" onclick="refreshScores()" style="padding: 8px 16px; font-size: 13px;">Показать</button>
      <button class="btn btn-secondary" onclick="switchAdminTab('score-weights')" style="padding: 8px 16px; font-size: 13px;">⚖️ Веса</button>
    </div>`;

  if (!report || !report.users || report.users.length === 0) {
    html += '<div class="empty-state">Нет данных за выбранный период</div>';
  } else {
    // Таблица рейтинга
    html += `<table class="score-table">
      <thead>
        <tr>
          <th>#</th>
          <th>Сотрудник</th>
          <th style="text-align: center;">Продуктивность</th>
          <th style="text-align: center;">Качество</th>
          <th style="text-align: center;">Инициатива</th>
          <th style="text-align: center;">Дисциплина</th>
          <th style="text-align: center;">Стрик</th>
    ...
    report.users.forEach((u, i) => {
      const streak = report.streakMap[u.user_id];
      const streakDays = streak ? streak.consecutive_on_time_days : 0;
      const streakBadge = streakDays >= 10 ? '🔥×1.25' : streakDays >= 5 ? '⚡×1.1' : streakDays > 0 ? `${streakDays}д` : '—';
      const streakColor = streakDays >= 10 ? 'var(--ios-red)' : streakDays >= 5 ? 'var(--ios-orange)' : 'var(--ios-gray)';

      html += `<tr onclick="showUserScoreHistory(${u.user_id}, '${u.user_name}')" style="cursor: pointer;">
        <td style="font-weight: 700; color: ${i < 3 ? 'var(--ios-orange)' : 'var(--ios-gray)'};">${i + 1}</td>
        <td><strong>${u.user_name}</strong></td>
        <td style="text-align: center; color: ${u.productivity > 0 ? 'var(--ios-green)' : 'var(--ios-gray)'};">${u.productivity || 0}</td>
        <td style="text-align: center; color: ${u.quality > 0 ? 'var(--ios-blue)' : 'var(--ios-gray)'};">${u.quality || 0}</td>
        <td style="text-align: center; color: ${u.initiative > 0 ? 'var(--ios-purple)' : 'var(--ios-gray)'};">${u.initiative || 0}</td>
        <td style="text-align: center; color: ${u.discipline >= 0 ? 'var(--ios-green)' : 'var(--ios-red)'};">${u.discipline || 0}</td>
        <td style="text-align: center; color: ${streakColor}; font-weight: 600;">${streakBadge}</td>
        <td style="text-align: center; font-weight: 700; font-size: 16px; color: ${u.grand_total >= 0 ? 'var(--ios-text)' : 'var(--ios-red)'};">${u.grand_total >= 0 ? '+' : ''}${u.grand_total}</td>
      </tr>`;
    });
  ...
  html += `<div id="userScoreHistory"></div></div>`;
  content.innerHTML = html;
}

async function refreshScores() {
  const start = document.getElementById('scoreStart').value;
  const end = document.getElementById('scoreEnd').value;
  if (!start || !end) return;

  const report = await apiGet(`/api/scores/report?startDate=${start}&endDate=${end}`);
  const container = document.getElementById('adminContent');
  if (!container) return;

  // Перезагружаем всю вкладку
  adminActiveTab = 'scores';
  switchAdminTab('scores');
}

async function showUserScoreHistory(userId, userName) {
  const container = document.getElementById('userScoreHistory');
  if (!container) return;

  container.innerHTML = '<div class="loading">Загрузка...</div>';
  const history = await apiGet(`/api/scores/history/${userId}`);
  ...
  let html = `<div style="margin-top: 20px; padding: 16px; background: var(--ios-card); border-radius: 12px;">
    <h3 style="margin-bottom: 12px;">📋 История: ${userName}</h3>
    <table style="width: 100%; font-size: 12px; border-collapse: collapse;">
      <thead><tr style="border-bottom: 2px solid var(--ios-border);">
        <th style="text-align: left; padding: 4px;">Время</th>
        <th style="text-align: left; padding: 4px;">Действие</th>
        <th style="text-align: center; padding: 4px;">Корзина</th>
        <th style="text-align: center; padding: 4px;">Сырьё</th>
        <th style="text-align: center; padding: 4px;">Множ.</th>
        <th style="text-align: center; padding: 4px;">Итого</th>
      </tr></thead>
      <tbody>`;

  history.forEach(h => {
    const color = h.points >= 0 ? 'var(--ios-green)' : 'var(--ios-red)';
    const sign = h.points >= 0 ? '+' : '';
    html += `<tr style="border-bottom: 1px solid var(--ios-border2);">
      <td style="padding: 4px;">${new Date(h.created_at).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</td>
      <td style="padding: 4px;">${h.label}</td>
      <td style="padding: 4px; text-align: center;">${h.basket}</td>
      <td style="padding: 4px; text-align: center;">${h.raw_points}</td>
      <td style="padding: 4px; text-align: center;">×${h.streak_multiplier}</td>
      <td style="padding: 4px; text-align: center; font-weight: 700; color: ${color};">${sign}${h.points}</td>
    </tr>`;
  });
...
// Веса баллов — отдельная вкладка админа
async function loadAdminScoreWeights(content) {
  const weights = await apiGet('/api/scores/weights');

  if (!weights || weights.length === 0) {
    content.innerHTML = '<div class="empty-state">Нет настроек весов</div>';
    return;
  }
  ...
  let html = `<div style="padding: 16px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
      <h3>⚖️ Настройка баллов</h3>
      <button class="btn btn-secondary" onclick="switchAdminTab('scores')" style="padding: 4px 12px; font-size: 13px;">‹ Назад к рейтингу</button>
    </div>`;

  weights.forEach(w => {
    if (w.basket !== currentBasket) {
      currentBasket = w.basket;
      const basketNames = { productivity: 'Продуктивность', quality: 'Качество', initiative: 'Инициатива', discipline: 'Дисциплина', bonus: 'Бонусы' };
      html += `<div style="padding: 8px 0; font-weight: 700; color: var(--ios-blue); margin-top: 12px; text-transform: uppercase; font-size: 12px;">${basketNames[w.basket] || w.basket}</div>`;
    }

    html += `<div class="shift-history-item" style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
      <span style="flex: 1; min-width: 150px;">${w.label}</span>
      <input type="number" id="sw-points-${w.id}" value="${w.points}" style="width: 60px; padding: 4px; border: 1px solid var(--ios-border); border-radius: 6px; text-align: center;">
      <span style="font-size: 12px; color: var(--ios-gray);">балл</span>
      <input type="number" id="sw-limit-${w.id}" value="${w.daily_limit}" style="width: 60px; padding: 4px; border: 1px solid var(--ios-border); border-radius: 6px; text-align: center;">
      <span style="font-size: 12px; color: var(--ios-gray);">/день</span>
      <label style="display: flex; align-items: center; gap: 4px; font-size: 12px;">
        <input type="checkbox" id="sw-active-${w.id}" ${w.is_active ? 'checked' : ''}> Активно
      </label>
      <button class="btn btn-primary" style="padding: 2px 8px; font-size: 11px;" onclick="saveWeight(${w.id})">💾</button>
    </div>`;
  });
...
async function saveWeight(id) {
  const points = parseInt(document.getElementById(`sw-points-${id}`).value);
  const dailyLimit = parseInt(document.getElementById(`sw-limit-${id}`).value);
  const isActive = document.getElementById(`sw-active-${id}`).checked ? 1 : 0;

  await apiPut(`/api/scores/weights/${id}`, { points, daily_limit: dailyLimit, is_active: isActive });
  // Мигаем зелёным
...
async function loadAdminPush(content) {
  const [subs, users] = await Promise.all([
    apiGet('/api/push/subscriptions'),
    apiGet('/api/admin/users')
  ]);

  let html = `<div style="padding: 16px;">
    <div style="margin-bottom: 16px; padding: 12px; background: var(--ios-card); border-radius: 12px;">
      <div style="font-weight: 600; margin-bottom: 8px;">📊 Подписчиков: ${subs ? subs.length : 0}</div>
      ${subs && subs.length > 0 ? `<div style="font-size: 13px; color: var(--ios-gray);">
        ${subs.map(s => `${s.user_name || 'Неизвестный'} — ${s.created_at ? new Date(s.created_at).toLocaleDateString('ru-RU') : ''}`).join('<br>')}
      </div>` : '<div style="font-size: 13px; color: var(--ios-gray);">Нет подписчиков</div>'}
    </div>

    <div style="margin-bottom: 20px; padding: 16px; background: var(--ios-card); border-radius: 12px;">
      <h3 style="margin-bottom: 12px; font-size: 16px;">📢 Общее уведомление</h3>
      <input type="text" id="pushAllTitle" class="form-input" placeholder="Заголовок" style="margin-bottom: 8px;">
      <textarea id="pushAllBody" class="form-input" placeholder="Текст уведомления" style="margin-bottom: 12px; min-height: 60px;"></textarea>
      <button class="btn btn-primary" onclick="sendPushAll()" style="width: 100%;">Отправить всем</button>
    </div>

    <div style="padding: 16px; background: var(--ios-card); border-radius: 12px;">
      <h3 style="margin-bottom: 12px; font-size: 16px;">👤 Личное уведомление</h3>
      <select id="pushUserId" class="form-input" style="margin-bottom: 8px;">
        <option value="">Выберите сотрудника</option>
        ${(users || []).map(u => `<option value="${u.id}">${u.login}</option>`).join('')}
      </select>
      <input type="text" id="pushUserTitle" class="form-input" placeholder="Заголовок" style="margin-bottom: 8px;">
      <textarea id="pushUserBody" class="form-input" placeholder="Текст уведомления" style="margin-bottom: 12px; min-height: 60px;"></textarea>
      <button class="btn btn-primary" onclick="sendPushToUser()" style="width: 100%;">Отправить сотруднику</button>
    </div>
  </div>`;
...
async function sendPushAll() {
  const title = document.getElementById('pushAllTitle').value;
  const body = document.getElementById('pushAllBody').value;
  if (!title || !body) { alert('Заполните заголовок и текст'); return; }

  const result = await apiPost('/api/push/send-all', { title, body });
  if (result.success) {
    alert(`Отправлено: ${result.sent}, Ошибок: ${result.failed}`);
    document.getElementById('pushAllTitle').value = '';
    document.getElementById('pushAllBody').value = '';
  } else {
    alert(result.error || 'Ошибка');
  }
}
...
async function sendPushToUser() {
  ...
  const title = document.getElementById('pushUserTitle').value;
  const body = document.getElementById('pushUserBody').value;
  if (!userId || !title || !body) { alert('Заполните все поля'); return; }

  const result = await apiPost('/api/push/send-user', { userId: parseInt(userId), title, body });
  if (result.success) {
    alert(`Отправлено: ${result.sent}, Ошибок: ${result.failed}`);
    document.getElementById('pushUserTitle').value = '';
    document.getElementById('pushUserBody').value = '';
  } else {
    alert(result.error || 'Ошибка');
  }
}
...
async function triggerSync() {
  const result = await apiPost('/api/sync/start', {});
  if (result.success) {
    alert('Синхронизация каталога запущена. Это может занять несколько минут.');
    // Poll for status
    pollSyncStatus();
  } else {
    alert('Ошибка: ' + (result.error || 'Не удалось запустить'));
  }
}

async function pollSyncStatus() {
  const statusEl = document.getElementById('lastSyncDate');
  let attempts = 0;
  ...
  const interval = setInterval(async () => {
    ...
    if (data) {
      if (statusEl) {
        statusEl.textContent = `${data.stage || ''}: ${data.message || ''} (${data.progress || 0}%)`;
      }
      if (data.status === 'completed') {
        clearInterval(interval);
        if (statusEl) {
          const d = new Date(data.last_sync_at);
          statusEl.textContent = `Каталог обновлён: ${d.toLocaleDateString('ru-RU')} ${d.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}`;
        }
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 400px;">
      ...
      <div style="margin-bottom: 8px;">
        <label style="font-size: 13px; color: var(--ios-gray); display: block; margin-bottom: 4px;">Таймер (часы, 0 = без таймера, макс 600)</label>
        <input type="number" id="todoTimer" class="form-input" placeholder="0" min="0" max="600" value="0" style="margin-bottom: 8px;">
      </div>
...
async function createTodo() {
  ...
  const timerHours = parseInt(document.getElementById('todoTimer').value) || 0;
  ...
  if (timerHours < 0 || timerHours > 600) { alert('Таймер: 0-600 часов'); return; }
  
  await apiPost('/api/calendar/todos', { date, title, description, assigneeIds, timerHours });
...
async function loadExpiryCheckData() {
  const products = await apiGet(`/api/expiry-check/expiring-soon?days=${expiryFilterDays}`);
  const list = document.getElementById('expiryList');
  if (!list) return;
  
  // Обновляем текст фильтра
  const filterLabel = document.getElementById('expiryFilterLabel');
  if (filterLabel) {
    filterLabel.textContent = `Товары со сроком < ${expiryFilterDays} дней`;
  }
  ...
  products.forEach(p => {
    ...
    const urgencyColor = daysUntil <= 7 ? 'var(--ios-red)' : daysUntil <= 30 ? 'var(--ios-orange)' : daysUntil <= 60 ? 'var(--ios-yellow)' : 'var(--ios-blue)';
    const isChecked = p.is_confirmed;
    const hasNewExpiry = p.new_expiry && p.new_expiry !== p.original_expiry;
    ...
    html += `<div class="expiry-item ${isChecked ? 'confirmed' : ''}" data-product-id="${p.id}">
      <div class="expiry-info">
        ...
        ${hasNewExpiry && p.new_expiry ? `<div style="font-size: 12px; color: var(--ios-green); font-weight: 500;">
          ✓ Новый срок: ${p.new_expiry}${p.checked_by_name ? ` (${p.checked_by_name})` : ''}
        </div>` : ''}
        ${isChecked && !hasNewExpiry ? `<div style="font-size: 12px; color: var(--ios-green);">✓ Подтверждено${p.checked_by_name ? ` — ${p.checked_by_name}` : ''}</div>` : ''}
      </div>
      <div class="expiry-actions">
        ${isChecked ? '<span style="color: var(--ios-green); font-size: 18px; margin-right: 8px;">✓</span>' : ''}
        <button class="btn btn-secondary" style="padding: 4px 12px; font-size: 12px;" onclick="openExpiryModal(${p.id}, '${p.name.replace(/'/g, "\\'")}', '${p.expiry_date || ''}', '${(p.new_expiry || '').replace(/'/g, "\\'")}')">
...
function renderExpiryCheck() {
  return `<div class="main-layout">
    <div class="header">
      ...
      <button class="btn btn-secondary" onclick="printExpiryCheck()" style="padding: 8px 16px; font-size: 14px;">🖨️ Печать</button>
    </div>
    <div style="padding: 8px 16px; background: var(--ios-card); text-align: center;">
      <div id="expiryFilterLabel" style="font-size: 13px; color: var(--ios-gray); margin-bottom: 8px;">Товары со сроком < 60 дней</div>
      <div style="display: flex; gap: 8px; justify-content: center;">
        <button class="btn ${expiryFilterDays === 30 ? 'btn-primary' : 'btn-secondary'}" style="padding: 4px 12px; font-size: 12px;" onclick="setExpiryFilter(30)">< 30 дн</button>
        <button class="btn ${expiryFilterDays === 60 ? 'btn-primary' : 'btn-secondary'}" style="padding: 4px 12px; font-size: 12px;" onclick="setExpiryFilter(60)">< 60 дн</button>
        <button class="btn ${expiryFilterDays === 90 ? 'btn-primary' : 'btn-secondary'}" style="padding: 4px 12px; font-size: 12px;" onclick="setExpiryFilter(90)">< 90 дн</button>
      </div>
    </div>
...
async function setExpiryFilter(days) {
  expiryFilterDays = days;
  renderApp();
  await loadExpiryCheckData();
}

function openExpiryModal(productId, productName, currentExpiry, savedNewExpiry) {
  const modal = document.createElement('div');
  modal.className = 'modal-overlay';
  modal.id = 'expiryCheckModal';
  
  const expiryFormatted = formatExpiryDate(currentExpiry);
  const savedExpiryFormatted = savedNewExpiry ? formatExpiryDate(savedNewExpiry) : '';
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 380px;">
      ...
      <div style="margin-bottom: 12px;">
        <label style="font-size: 13px; color: var(--ios-gray); display: block; margin-bottom: 4px;">Новый срок годности</label>
        <input type="text" id="newExpiryInput" class="form-input" placeholder="Например: 31.12.25 или 2025-12-31" style="margin-bottom: 8px;" value="${savedExpiryFormatted || expiryFormatted || ''}">
      </div>
      <div class="modal-actions">
...
// Печать проверки сроков
async function printExpiryCheck() {
  const data = await apiGet(`/api/expiry-check/print?days=${expiryFilterDays}`);
  if (!data || !data.items || data.items.length === 0) {
    alert('Нет товаров для печати');
    return;
  }
  ...
  const printWindow = window.open('', '_blank');
  printWindow.document.write(`
    <html><head><title>Проверка сроков</title>
    <style>
      body{font-family:Arial,sans-serif;padding:20px;font-size:13px}
      h2{margin-bottom:5px}
      table{width:100%;border-collapse:collapse;font-size:12px}
      th,td{padding:6px;border-bottom:1px solid #ddd;text-align:left}
      th{border-bottom:2px solid #333;background:#f5f5f5}
      .critical{color:#FF3B30;font-weight:bold}
      .warning{color:#FF9500;font-weight:bold}
      .attention{color:#007AFF}
      .checked{background:rgba(52,199,89,0.1)}
      @media print{
        .no-print{display:none}
        body{padding:0}
      }
    </style></head><body>
    <button class="no-print" onclick="window.print()" style="padding:8px 16px;margin-bottom:12px;">🖨️ Печать</button>
    <h2>Проверка сроков годности</h2>
    <p style="color:#666">Дата: ${data.date} | Фильтр: < ${data.days} дней | Всего: ${data.total} | Проверено: ${data.checked}</p>
    <table>
      <thead><tr>
        <th>№</th>
        <th>Артикул</th>
      ...
      <tbody>
        ${data.items.map((item, i) => {
          ...
          return `<tr class="${isChecked ? 'checked' : ''}">
            <td>${i + 1}</td>
            <td style="font-weight:bold">${item.vendor_code || '-'}</td>
            <td style="font-weight:bold">${item.name}</td>
            <td>${item.category_name}</td>
            <td>${item.expiry_date}</td>
            <td class="${statusClass}">${item.days_remaining} дн.</td>
            <td style="color:${item.new_expiry ? '#34C759' : '#999'}">${item.new_expiry || '—'}</td>
            <td>${item.checked_by_name || '—'}</td>
            <td style="text-align:center;font-size:16px">${isChecked ? '✓' : '○'}</td>
          </tr>`;
        }).join('')}
      </tbody>
    </table>
...
let specialTasksTab = 'my'; // 'my' или 'all'
...
async function loadSpecialTasks() {
  ...
  // Для админа на вкладке 'all' грузим все, иначе свои
  const endpoint = (isAdmin && specialTasksTab === 'all') 
    ? '/api/special-tasks/all' 
    : '/api/special-tasks/my';
  
  const [tasks, scale] = await Promise.all([
    apiGet(endpoint),
  ...
  // Рендерим табы для админа
  const tabsContainer = document.getElementById('specialTasksTabs');
  if (tabsContainer && isAdmin) {
    tabsContainer.innerHTML = `
      <div class="admin-nav" style="padding: 0 16px; margin-bottom: 8px;">
        <div class="admin-tab ${specialTasksTab === 'my' ? 'active' : ''}" onclick="switchSpecialTasksTab('my')">Мои</div>
        <div class="admin-tab ${specialTasksTab === 'all' ? 'active' : ''}" onclick="switchSpecialTasksTab('all')">Все сотрудники</div>
      </div>
    `;
  }
  ...
  list.innerHTML = tasks.map(t => renderSpecialTaskCard(t, isAdmin && specialTasksTab === 'all')).join('');
}

function renderSpecialTaskCard(t, isAdminView) {
  ...
  let adminActions = '';
  if (isAdminView && t.status === 'pending') {
    adminActions = `<div style="display: flex; gap: 8px; margin-top: 10px;">
      <button class="btn btn-success" style="flex: 1; padding: 6px; font-size: 13px;" onclick="resolveSpecialTask(${t.id}, 'approved')">✓ Одобрить</button>
      <button class="btn btn-danger" style="flex: 1; padding: 6px; font-size: 13px;" onclick="resolveSpecialTask(${t.id}, 'rejected')">✗ Отклонить</button>
    </div>
    <input type="text" id="st-comment-${t.id}" class="form-input" placeholder="Комментарий админа" style="margin-top: 6px; font-size: 13px;">`;
  }
  ...
  return `<div class="special-task-item">
    ...
    ${isAdminView ? `<div style="font-size: 12px; color: var(--ios-blue); margin-bottom: 4px;">👤 ${t.user_name || 'Неизвестный'}</div>` : ''}
...
function switchSpecialTasksTab(tab) {
  specialTasksTab = tab;
  loadSpecialTasks();
}

async function resolveSpecialTask(taskId, status) {
  const commentInput = document.getElementById(`st-comment-${taskId}`);
  const comment = commentInput ? commentInput.value : '';
  await apiPost(`/api/special-tasks/${taskId}/resolve`, { status, comment });
  loadSpecialTasks();
}
...
function renderSpecialTasks() {
  ...
  return `<div class="main-layout">
    ...
    ${isAdmin ? '<div id="specialTasksTabs"></div>' : ''}
    <div style="padding: 16px;">
      <button class="btn btn-primary" onclick="showCreateSpecialTaskModal()" style="width: 100%; margin-bottom: 8px;">+ Новое задание</button>
    </div>
...
async function loadProfileData() {
  ...
  if (docsContainer) {
    let docsHtml = `
      <div style="margin-bottom: 16px;">
        <input type="file" id="docUploadInput" style="display: none;" onchange="uploadDocument(this)">
        <input type="text" id="docTitleInput" class="form-input" placeholder="Название документа" style="margin-bottom: 8px;">
        <button class="btn btn-primary" style="width: 100%;" onclick="triggerDocUpload()">📎 Загрузить документ</button>
      </div>
    `;
    
    if (!documents || documents.length === 0) {
      docsHtml += '<div class="empty-state">Нет загруженных документов</div>';
    } else {
      docsHtml += documents.map(d => `<div class="shift-history-item" style="display: flex; justify-content: space-between; align-items: center;">
        <span>📄 ${d.title}</span>
...
function renderProfile() {
  ...
  return `<div class="main-layout">
    ...
    <div style="padding: 16px; border-top: 1px solid var(--ios-border);">
      <button class="btn btn-primary" onclick="showRequestModal('advance')" style="width: 100%;">💰 Запросить аванс</button>
    </div>
    <div style="padding: 16px; text-align: center; border-top: 1px solid var(--ios-border);">
      <button id="pushToggleBtn" class="btn btn-secondary" style="padding: 8px 16px; font-size: 13px; width: 100%; margin-bottom: 8px;" onclick="subscribeToPush()">🔕 Включить уведомления</button>
...
async function uploadDocument(input) {
  ...
  if (!title) { alert('Введите название документа'); return; }
  
  const reader = new FileReader();
  reader.onload = async function(e) {
    await apiPost('/api/profile/documents', {
      title: title,
      fileData: e.target.result,
      fileType: file.type,
      fileName: file.name
...
async function showChat() {
  ...
  await apiPost('/api/profile/chat/mark-read', {});
  unreadChatCount = 0;
  updateChatBadge();
}
  ...
  if (chatMessages.length === 0) {
    ...
    container.innerHTML = chatMessages.map(m => {
      ...
      return `<div class="chat-message ${isMe ? 'sent' : 'received'}">
        ${!isMe ? `<span class="chat-sender">${m.user_name || 'Неизвестный'}</span>` : ''}
...
async function subscribeToPush() {
  ...
  try {
    const registration = await navigator.serviceWorker.ready;
    const vapidPublicKey = await apiGet('/api/push/vapid-public-key');

    if (!vapidPublicKey || !vapidPublicKey.key) {
    ...
    const subscription = await registration.pushManager.subscribe({
      ...
      applicationServerKey: urlBase64ToUint8Array(vapidPublicKey.key)
    });
    ...
    updatePushUI();
    alert('Вы подписались на push-уведомления!');
  } catch (e) {
