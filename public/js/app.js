...
function renderPriceCheckGrid(products) {
  ...
  grid.innerHTML = products.map(p => {
    ...
    // Отладка - показываем срок годности если есть
    const expiryDebug = p.expiry_date ? `| ${expiryFormatted}` : '';
    ...
    return `<div class="price-check-card" onclick="openPriceCheckModal(${p.id}, '${p.name.replace(/'/g, "\\'")}', '${p.vendor_code || ''}', '${p.expiry_date || ''}')">
      <div class="price-check-image-wrapper">
        ...
        ${trafficLight ? `<div class="expiry-light" style="background: ${trafficLight}" title="Срок годности: ${expiryFormatted || 'не указан'}"></div>` : ''}
      </div>
      <div class="price-check-info">
        <div class="price-check-name">${p.name}</div>
        <div class="price-check-code">${p.vendor_code || ''}</div>
        ${expiryFormatted ? `<div class="price-check-expiry">${expiryFormatted}</div>` : '<div class="price-check-expiry">Нет срока</div>'}
      </div>
    </div>`;
  }).join('');
}
...
async function printPriceCheck() {
  ...
  window.printData = {
    ...
    items: data.items.map(item => ({
      ...
      need_new_tag: item.mark_type === 'need_new_tag' ? '✓' : '',
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
        <thead>
          <tr style="border-bottom: 2px solid #333;">
            ...
            <th style="text-align: center; padding: 6px; font-size: 10px;">Нужен новый<br>ценник</th>
        ...
        <tbody>
          ${data.items.map((item, i) => `<tr style="border-bottom: 1px solid #ddd;">
            ...
            <td style="padding: 6px; text-align: center; color: ${item.need_new_tag ? 'var(--ios-red)' : '#ccc'}; font-weight: bold;">${item.need_new_tag || '○'}</td>
