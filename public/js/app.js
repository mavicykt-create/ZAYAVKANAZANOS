...
function renderAssemblyList(items) {
  ...
  container.innerHTML = items.map(item => `<div class="assembly-item"><div class="assembly-checkbox" id="cb-${item.id}" onclick="toggleCollected(${item.id})"></div><div class="assembly-content"><div style="font-weight: 500;">${item.product_name}</div><div class="product-code">${item.vendor_code || ''}</div></div><div class="assembly-qty">${item.quantity}</div></div>`).join('');
}
...
async function loadAdminOverview() {
  ...
  if (container && data) {
    ...
    <div class="stat-row"><span class="stat-label">Статус синхронизации</span><span class="stat-value" style="color: ${data.syncStatus?.status === 'completed' ? 'var(--ios-green)' : 'var(--ios-orange)'}">${data.syncStatus?.status || 'idle'}</span></div>`;
  }
}
