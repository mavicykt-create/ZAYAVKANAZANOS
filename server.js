...
const IMAGE_CACHE_VERSION = process.env.IMAGE_CACHE_VERSION || 'v4';
...
async function getSyncState() {
  const rows = await all(`SELECT key, value FROM app_state WHERE key IN (
    'sync_running','sync_progress','sync_stage','sync_message','sync_last_started_at','sync_last_finished_at','sync_total_offers','sync_processed_offers'
  )`);
...
async function startSyncState() {
  ...
  await setAppState('sync_last_started_at', Date.now());
...
async function releaseExpiredLocks() {
  ...
  await run(`UPDATE active_order_categories
    SET status = 'open', locked_by = NULL, locked_at = NULL
    WHERE status = 'locked' AND locked_at IS NOT NULL AND locked_at < ?`, [cutoff]);
}

function imageCachePath(url) {
  const hash = crypto.createHash('sha1').update(`${IMAGE_CACHE_VERSION}|${IMAGE_WIDTH}|${IMAGE_QUALITY}|${url}`).digest('hex');
...
async function ensureCompressedImage(url) {
  ...
  try {
    const response = await fetch(url, {
      headers: { 'User-Agent': 'warehouse-order-app/2.0' },
      signal: controller.signal,
    });
    if (!response.ok) throw new Error(`Не удалось скачать картинку: HTTP ${response.status}`);
...
async function syncCatalogFromYml() {
  ...
  try {
    const response = await fetch(YML_URL, { headers: { 'User-Agent': 'warehouse-order-app/2.0' } });
    if (!response.ok) throw new Error(`Не удалось скачать YML: HTTP ${response.status}`);
...
async function getFullState() {
  ...
  const categories = await all(`SELECT c.id, c.name, a.status, a.locked_by, a.locked_at, a.completed_at
     FROM catalog_categories c
     JOIN active_order_categories a ON a.category_id = c.id
     ORDER BY c.sort_order ASC, c.id ASC`);

  const products = await all(`SELECT p.id, p.category_id, p.name, p.vendor_code, p.picture, p.price, p.stock_quantity,
  ...
  const orderedItems = products.filter((p) => Number(p.qty) > 0);
  ...
  return {
    ...
    sync: { ...sync, nextAllowedAt: nextSyncAllowedAt(sync) },
    imageSettings: { width: IMAGE_WIDTH, quality: IMAGE_QUALITY, cacheVersion: IMAGE_CACHE_VERSION },
  };
}
...
app.get('/api/image', requireAuth, async (req, res) => {
  try {
    ...
    return json(res, false, { error: 'Ошибка обработки картинки' }, 500);
  }
});
...
app.post('/api/sync-yml', requireAuth, async (req, res) => {
  try {
    ...
    if (sync.running || syncPromise) {
      const state = await getFullState();
      return json(res, false, { error: 'Обновление каталога уже выполняется', state }, 409);
    }
    const nextAllowedAt = nextSyncAllowedAt(sync);
    if (nextAllowedAt && Date.now() < nextAllowedAt) {
      const waitMinutes = Math.ceil((nextAllowedAt - Date.now()) / 60000);
      const state = await getFullState();
      return json(res, false, { error: `Каталог можно обновлять только 1 раз в час. Подождите ещё ${waitMinutes} мин.`, state }, 429);
    }
    syncPromise = syncCatalogFromYml().finally(() => { syncPromise = null; });
    const state = await getFullState();
...
app.post('/api/sync-reset', requireAuth, async (req, res) => {
  ...
  const state = await getFullState();
  return json(res, true, { message: 'Состояние обновления каталога сброшено', state });
});
...
app.post('/api/categories/:id/lock', requireAuth, async (req, res) => {
  ...
  const result = await run(`UPDATE active_order_categories SET status = 'locked', locked_by = ?, locked_at = ?, completed_at = NULL
    WHERE category_id = ? AND (status = 'open' OR locked_by = ?)` , [req.sessionUser.userId, Date.now(), categoryId, req.sessionUser.userId]);
  if (result.changes === 0) {
    const state = await getFullState();
    return json(res, false, { error: 'Категория занята другим сотрудником', state }, 409);
  }
  const state = await getFullState();
  return json(res, true, { state });
});
...
app.post('/api/categories/:id/unlock', requireAuth, async (req, res) => {
  ...
  await run(`UPDATE active_order_categories SET status = 'open', locked_by = NULL, locked_at = NULL WHERE category_id = ?`, [categoryId]);
  const state = await getFullState();
...
app.post('/api/categories/:id/heartbeat', requireAuth, async (req, res) => {
  const categoryId = Number(req.params.id);
  await run(`UPDATE active_order_categories SET locked_at = ? WHERE category_id = ? AND locked_by = ? AND status = 'locked'`, [Date.now(), categoryId, req.sessionUser.userId]);
...
app.post('/api/categories/:id/complete', requireAuth, async (req, res) => {
  ...
  await run(`UPDATE active_order_categories SET status = 'completed', locked_by = NULL, locked_at = NULL, completed_at = ? WHERE category_id = ?`, [Date.now(), categoryId]);
  const state = await getFullState();
  return json(res, true, { state });
});
...
app.post('/api/items/:id/increment', requireAuth, async (req, res) => {
  ...
  await run(`INSERT INTO active_order_items (product_id, qty, picked) VALUES (?, 1, 0)
     ON CONFLICT(product_id) DO UPDATE SET qty = qty + 1, picked = 0`, [productId]);
  const state = await getFullState();
  return json(res, true, { state });
});
...
app.post('/api/items/:id/decrement', requireAuth, async (req, res) => {
  ...
  if (!row) return json(res, true, { state: await getFullState() });
  const nextQty = Math.max(0, Number(row.qty) - 1);
  if (nextQty === 0) await run(`DELETE FROM active_order_items WHERE product_id = ?`, [productId]);
  else await run(`UPDATE active_order_items SET qty = ?, picked = 0 WHERE product_id = ?`, [nextQty, productId]);
  const state = await getFullState();
  return json(res, true, { state });
});
...
app.post('/api/items/:id/toggle-picked', requireAuth, async (req, res) => {
  ...
  if (!row || Number(row.qty) <= 0) return json(res, false, { error: 'Товар не заказан' }, 400);
