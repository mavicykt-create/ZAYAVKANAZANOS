...
## Deploy на Amvera
Используется Docker-схема, чтобы зависимости гарантированно попадали в runtime-контейнер.

Файлы для деплоя:
- `Dockerfile`
- `amvera.yml`

Это специально сделано для обхода ошибки вида `Cannot find package ... imported from /app/src/app.js`, когда native node-сборка на платформе не переносит `node_modules` в runtime как ожидалось.
