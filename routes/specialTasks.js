...
router.post('/:id/resolve', requireAuth, requireAdmin, (req, res) => {
  ...
  // Получаем задачу для начисления баллов создателю
  const task = db.prepare('SELECT user_id, description FROM special_tasks WHERE id = ?').get(taskId);
  ...
  // Баллы за инициативу (начисляем создателю задания при одобрении)
  if (status === 'approved' && task) {
    awardScore(task.user_id, 'special_task_approved', { entityId: taskId, label: task.description.substring(0, 50) });
  }
