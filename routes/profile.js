...
router.get('/chat/unread-count', requireAuth, (req, res) => {
  ...
  const readStatus = db.prepare('SELECT last_read_at FROM chat_read_status WHERE user_id = ?').get(userId);
  const lastRead = readStatus ? readStatus.last_read_at : '1970-01-01';
  ...
  res.json({ count: result.count, lastRead });
});

// Отметить чат как прочитанный
router.post('/chat/mark-read', requireAuth, (req, res) => {
  const db = getDB();
  const userId = req.session.userId;

  db.prepare(`
    INSERT OR REPLACE INTO chat_read_status (user_id, last_read_at)
    VALUES (?, CURRENT_TIMESTAMP)
  `).run(userId);
