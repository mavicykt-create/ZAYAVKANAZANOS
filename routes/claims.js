...
router.post('/:id/resolve', requireAuth, (req, res) => {
  ...
  // Find the task
  const task = db.prepare('SELECT * FROM claim_tasks WHERE claim_id = ? AND assigned_to = ?').get(claimId, userId);
  if (!task) {
    return res.status(403).json({ error: 'Вы не назначены на эту претензию' });
  }
...
router.post('/:id/evidence', requireAuth, (req, res) => {
  ...
  const task = db.prepare('SELECT id FROM claim_tasks WHERE claim_id = ? AND assigned_to = ?').get(claimId, userId);
  if (!task) {
    return res.status(403).json({ error: 'Вы не назначены на эту претензию' });
  }
  ...
  db.prepare(`
    ...
    WHERE claim_id = ? AND assigned_to = ?
  `).run(relativePath, fileType || 'image', claimId, userId);
