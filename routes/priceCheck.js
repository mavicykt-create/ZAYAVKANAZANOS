...
router.post('/mark-product', requireAuth, (req, res) => {
  const { productId, markType } = req.body;
  const userId = req.session.userId;
  const db = getDB();
  ...
  if (markType === 'remove') {
    ...
    // Add or update mark
    db.prepare(`
      INSERT OR REPLACE INTO price_check_marks (product_id, user_id, mark_type)
      VALUES (?, ?, ?)
    `).run(productId, userId, markType);
  }
  
  // Log action
  db.prepare('INSERT INTO user_actions_log (user_id, action_type, details) VALUES (?, ?, ?)')
    .run(userId, 'price_check_mark', JSON.stringify({ productId, markType }));
