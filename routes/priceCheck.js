...
// ===== НОВАЯ ЛОГИКА ПРОВЕРКИ ЦЕННИКОВ =====

const NEW_ITEMS_PER_PAGE = 48; // 4 в ряд * 12 строк

// Get all products for new price check (all categories, ordered)
router.get('/all-products', requireAuth, (req, res) => {
  const db = getDB();
  const { page = 1 } = req.query;
  const offset = (parseInt(page) - 1) * NEW_ITEMS_PER_PAGE;
  ...
  const products = db.prepare(`
    ...
    LEFT JOIN price_check_marks pcm ON p.id = pcm.product_id
    ORDER BY c.name, p.name
    LIMIT ? OFFSET ?
  `).all(NEW_ITEMS_PER_PAGE, offset);
  
  res.json({
    products,
    totalCount,
    totalPages: Math.ceil(totalCount / NEW_ITEMS_PER_PAGE),
    currentPage: parseInt(page)
  });
});

// Get total count of all products
router.get('/total-count', requireAuth, (req, res) => {
  const db = getDB();
  const count = db.prepare('SELECT COUNT(*) as count FROM products').get().count;
  res.json({ count });
});

// Mark product with problem type
router.post('/mark-product', requireAuth, (req, res) => {
  const { productId, markType, newExpiry } = req.body;
  const userId = req.session.userId;
  const db = getDB();
  
  if (markType === 'remove') {
    // Remove mark
    db.prepare('DELETE FROM price_check_marks WHERE product_id = ?').run(productId);
  } else {
    // Add or update mark
    db.prepare(`
      INSERT OR REPLACE INTO price_check_marks (product_id, user_id, mark_type, new_expiry)
      VALUES (?, ?, ?, ?)
    `).run(productId, userId, markType, newExpiry || null);
  }
  
  // Log action
  db.prepare('INSERT INTO user_actions_log (user_id, action_type, details) VALUES (?, ?, ?)')
    .run(userId, 'price_check_mark', JSON.stringify({ productId, markType, newExpiry }));

  // Баллы за продуктивность (не за удаление отметки)
  if (markType !== 'remove') {
    awardScore(userId, 'price_check_mark', { entityId: productId, label: `Ценник: ${markType}` });
  }
...
// Get all marks
router.get('/all-marks', requireAuth, (req, res) => {
  const db = getDB();
  
  const marks = db.prepare(`
    SELECT pcm.product_id, pcm.mark_type, pcm.user_id, u.login
    FROM price_check_marks pcm
...
// Clear all marks (admin only)
router.post('/clear-marks', requireAuth, (req, res) => {
  const db = getDB();
  const userId = req.session.userId;
  ...
  if (user.role !== 'admin') {
    return res.status(403).json({ error: 'Только администратор может очистить отметки' });
  }
  
  db.prepare('DELETE FROM price_check_marks').run();
  
  res.json({ success: true, message: 'Все отметки очищены' });
});

// Get print data for price check - ALL marked products, sorted by category then alphabetically
router.get('/print-check', requireAuth, (req, res) => {
  const db = getDB();
