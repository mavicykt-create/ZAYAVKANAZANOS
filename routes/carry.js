...
router.get('/requests', requireAuth, (req, res) => {
  ...
  const requests = db.prepare(`
    SELECT cr.*, p.name as product_name, p.vendor_code, p.picture,
           p.stock_quantity, p.box_count, p.block_count,
...
router.get('/print/:categoryId', requireAuth, (req, res) => {
  ...
  const items = db.prepare(`
    SELECT cr.quantity, p.name, p.vendor_code, p.stock_quantity, p.box_count, p.block_count, c.name as category_name
    FROM carry_requests cr
    JOIN products p ON cr.product_id = p.id
    JOIN categories c ON cr.category_id = c.id
...
router.post('/complete-category-collab', requireAuth, (req, res) => {
  ...
  awardScore(userId, 'carry_category', { entityId: categoryId, label: `Вынос категории #${categoryId}` });
